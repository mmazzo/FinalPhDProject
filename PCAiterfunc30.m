function [MUdata] = PCAiterfunc30(MUdata)
...
% --------- PCA of smoothed, detrended IDR lines - Whole 30s --------- 
        ...
        we = MUdata.start + (30*2000);
            ...
            % If < 3 MUs are active
            if size(idrdat,1) < 3
                MUdata.(mus).PCA.iter.w30.coeffs = {};
                MUdata.(mus).PCA.iter.w30.explained = {};
            else
            % Run PCA for that window
            [pcadat] = PCAiter(idrdat,100);
                if isstruct(pcadat)
                    MUdata.(mus).PCA.iter.w30.coeffs = pcadat.coeff_all;
                    MUdata.(mus).PCA.iter.w30.explained = pcadat.expl_all;
                    MUdata.(mus).PCA.iter.w30.coeffs_mean = pcadat.coeff_mean;
                    MUdata.(mus).PCA.iter.w30.explained_means = pcadat.expl_mean;
                else
                    disp(strcat(mus,{' '},'no pca'))
                end
            end
        rem = [];
        idrdat = [];
...
% ------------- For all PFs combined ---------------------------------
    ...
    we = MUdata.start + (30*2000);
            ...
            % If < 3 MUs are active
            if size(idrdat,1) < 3
                MUdata.PCA.iter.w30.coeffs{w} = {};
                MUdata.PCA.iter.w30.explained{w} = {};
            else
            % Run PCA for that window
            pcadat = PCAiter(idrdat,100);
                if isstruct(pcadat)
                    MUdata.PCA.iter.w30.coeffs = pcadat.coeff_all;
                    MUdata.PCA.iter.w30.explained = pcadat.expl_all;
                    MUdata.PCA.iter.w30.coeffs_mean = pcadat.coeff_mean;
                    MUdata.PCA.iter.w30.explained_means = pcadat.expl_mean;
                else
                end
            end
