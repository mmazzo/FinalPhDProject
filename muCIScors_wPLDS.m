...
if isnan(MUdat.cst)
    ...
    SmPLDS = conv(MUdat.PLDS.raw(ind3:ind4),hann_window,'same');
        MUdat.xcors.whole.SmPLDS{mu} = SmPLDS;
    % HPF   
    ...
    PLDS_hpf = filtfilt(filt2,filt1,SmPLDS);
        MUdat.xcors.whole.SmPLDS_hpf{mu} = PLDS_hpf;
    % ------------- Whole time active --------------------------------
        % PLDS xcorr
        [sigcor,siglag] = xcorr(SmMU_hpf,PLDS_hpf,'coeff');
            [maxcor,ind] = max(sigcor);
            MUdat.xcors.whole.PLDSmaxcors(mu,1) = maxcor;
            MUdat.xcors.whole.PLDSlag(mu,1) = siglag(ind);
        % CST xcorr
    ...
    MUdat.xcors.steady30.PLDS_hpf{mu} = MUdat.xcors.whole.SmPLDS_hpf{mu}(ind1:ind2);

    % Redo xcorrs
        % PLDS xcorr
        [sigcor,siglag] = xcorr(SmMU_hpf,PLDS_hpf,'coeff');
            [maxcor,ind] = max(sigcor);
            MUdat.xcors.steady30.PLDSmaxcors(mu,1) = maxcor;
            MUdat.xcors.steady30.PLDSlag(mu,1) = siglag(ind);
        % CST xcorr
        [sigcor,siglag] = xcorr(SmMU_hpf,CST_hpf,'coeff');
            [maxcor,ind] = max(sigcor);
