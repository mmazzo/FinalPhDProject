function [MUdat] = muresidnew_hpfonly(MUdat)  
    if isempty(MUdat)
    else
        ...
        for mu = 1:length(MUdat.MUPulses)
            ...
            if isempty(MUdat.MUPulses{1,mu})
                ...
                if 0 < sum(isnan(MUdat.rawlines{1,mu}(ind1:ind2)))
                    ...
                    residualsCST.h400.steady30.HPF.rawlines(mu,:) = filtfilt(filt2,filt1,MUdat.rawlines{1,mu}(ind1:ind2));
                    residualsCST.h400.steady30.HPF.cst = filtfilt(filt2,filt1,MUdat.cst(ind1:ind2));
                    ...
                    residualsCST.h150.steady30.HPF.rawlines(mu,:) = filtfilt(filt2,filt1,MUdat.rawlines{1,mu}(ind1:ind2));
                    residualsCST.h150.steady30.HPF.cst = filtfilt(filt2,filt1,MUdat.cst150(ind1:ind2));
                    ...
                    residualsPLDS.steady30.HPF.rawlines(mu,:) = filtfilt(filt2,filt1,MUdat.rawlines{1,mu}(ind1:ind2));
                    residualsPLDS.steady30.HPF.PLDS = filtfilt(filt2,filt1,MUdat.PLDS.raw(ind1:ind2));
                ...
                if isnan(MUdat.cst)
                    ...
                    tempflags{mu} = MUdat.flags(ind3:ind4);
                        if isnan(MUdat.cst)
                            ...
                            residualsCST.h400.HPF.rawlines{mu} = filtfilt(filt2, filt1,MUdat.rawlines{1,mu}(ind3:ind4));
                            residualsCST.h400.HPF.cst{mu} = filtfilt(filt2, filt1,MUdat.cst(ind3:ind4));
                            ...
                            residualsCST.h150.HPF.rawlines{mu} = filtfilt(filt2, filt1,MUdat.rawlines{1,mu}(ind3:ind4));
                            residualsCST.h150.HPF.cst{mu} = filtfilt(filt2, filt1,MUdat.cst150(ind3:ind4));
                            ...
                            residualsPLDS.HPF.rawlines{mu} = filtfilt(filt2, filt1,MUdat.rawlines{1,mu}(ind3:ind4));
                            residualsPLDS.HPF.PLDS{mu} = filtfilt(filt2, filt1,MUdat.PLDS.raw(ind3:ind4));
