...
new = [];
for w = 1:6
    temp = MUdata.PCA.w5.coeffs{1,w}(:,1)';
...
t = tiledlayout(3,1);
nexttile
title(t,'SFU01 All PFs Stretch Submax 10');
    yyaxis left
    plot(new,'r')
    ylim([-0.04 0.04]);
    yyaxis right
...
nexttile
    yyaxis left
    plot(highpass(MUdata.cst(MUdata.start:MUdata.endd),0.75,2000),'b')
    yyaxis right
    plot(new,'r')
    legend('CST','Coeffs');
        [sc,~] = xcorr(new,highpass(MUdata.cst(MUdata.start:MUdata.endd),0.75,2000),'coeff');
        [maxc,~] = max(sc);
        title(strcat('R= ',string(maxc)));
nexttile
    yyaxis left
    ...
    %plot(fdat.filt{1,1}(MUdata.start:MUdata.endd),'k')
    legend('CST','Force');
        [sc,~] = xcorr(fo,highpass(MUdata.cst(MUdata.start:MUdata.endd),0.75,2000),'coeff');
        [maxc,~] = max(sc);
        title(strcat('R= ',string(maxc)));
...
nexttile
    ...
    plot(highpass(coeff(:,1),0.75,2000),'r')
    legend('CST','Coeffs');
        [sc,~] = xcorr(highpass(MUdata.cst(MUdata.start:MUdata.endd),0.75,2000),highpass(coeff(:,1),0.75,2000),'coeff');
        [maxc,~] = max(sc);
        title(strcat('R= ',string(maxc)));
nexttile
    yyaxis left
    ...
    plot(highpass(fdat.filt{1,1}(MUdata.start:MUdata.endd),0.75,2000),'k')
    legend('CST','Force');
        [sc,~] = xcorr(highpass(MUdata.cst(MUdata.start:MUdata.endd),0.75,2000),highpass(fdat.filt{1,1}(MUdata.start:MUdata.endd),0.75,2000),'coeff');
        [maxc,~] = max(sc);
        title(strcat('R= ',string(maxc)));
        
%% pCSI
plot(PFdata.stretch.submax10.MUdata.pre.pCSI.w5.pCSI,'b')
hold on
plot(PFdata.stretch.submax10.MUdata.post.pCSI.w5.pCSI,'g')
xlim([0 20]);
ylim([0 1]);
ylabel('Coherence');
...
%% PCA iterations
p1 = plot(PFdata.stretch.submax10.MUdata.pre.PCA.iter.w5.fit,'b');
hold on;
p2 = plot(PFdata.stretch.submax10.MUdata.post.PCA.iter.w5.fit,'g');
ylim([0 100]);
xlim([0 33]);
ylabel('% Explained');
